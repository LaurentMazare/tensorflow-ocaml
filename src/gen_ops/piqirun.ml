...
let string_of_loc pos = string_of_int pos
let strerr loc s = string_of_loc loc ^ ": " ^ s
...
let error obj s =
  let loc = -1 in
  (* TODO, XXX: obj location db? *)
  buf_error loc s

type string_slice =
  { s : string
  ; start_pos : int
  ; (* position of `s` in the input stream *)
    len : int
  ; mutable pos : int
  }
...
let rec list_count_map f l ctr =
  ...
  | x1 :: x2 :: x3 :: x4 :: x5 :: tl ->
    ...
    :: f4
    :: f5
    :: (if ctr > 1000 then list_map_slow f tl else list_count_map f tl (ctr + 1))
...
module IBuf = struct
  ...
  let of_string x start_pos = String { s = x; len = String.length x; start_pos; pos = 0 }
  ...
  let to_string buf =
    ...
    | Channel x ->
      ...
      (try
         while true (* this cycle exist only on End_of_file exception *) do
  ...
  let next_byte buf =
    ...
    | String x ->
      if x.pos >= x.len
      then raise End_of_buffer
      else (
        let res = x.s.[x.pos] in
        x.pos <- x.pos + 1;
        Char.code res)
    | Channel x ->
      (try input_byte x with
      | End_of_file -> raise End_of_buffer)
  ...
  let next_block buf length =
    ...
    | Channel x ->
      ...
      (try Pervasives.really_input x s 0 length with
      | End_of_file -> raise End_of_buffer);
      of_string (Bytes.unsafe_to_string s) start_pos

  let of_string x = of_string x 0
end
...
let init_from_channel ch = Top_block (IBuf.of_channel ch)
let init_from_string s = Top_block (IBuf.of_string s)
let error_variant obj code = error obj ("unknown variant: " ^ string_of_int code)
let error_missing obj code = error obj ("missing field: " ^ string_of_int code)
...
let parse_varint64 i buf msb x partial_res =
  let rec aux i msb x res =
    ...
    then IBuf.error buf "integer overflow while reading varint"
    else (
      ...
      then Varint64 res (* no more octets => return *)
        ...
        aux (i + 1) msb x res))
    (* continue reading octets *)
  in
  aux i msb x (Int64.of_int partial_res)
...
let parse_varint_common buf i res =
  let rec aux i res =
    ...
    if y asr (i * 7) <> x
    then (* switch to Varint64 in case of overflow *)
      parse_varint64 i buf msb x res
    else (
      let res = res lor y in
      if msb = 0 then Varint res (* no more octets => return *) else aux (i + 1) res)
    (* continue reading octets *)
  in
  try aux i res with
  | IBuf.End_of_buffer -> IBuf.error buf "unexpected end of buffer while reading varint"

let parse_varint buf = parse_varint_common buf 0 0
...
let parse_fixed32 buf =
  try
    ...
    !res
  with
  | IBuf.End_of_buffer -> IBuf.error buf "unexpected end of buffer while reading fixed32"
...
let parse_fixed64 buf =
  try
    ...
    !res
  with
  | IBuf.End_of_buffer -> IBuf.error buf "unexpected end of buffer while reading fixed64"
...
let parse_block buf =
  ...
  | Varint length when length >= 0 ->
    (try IBuf.next_block buf length with
    | IBuf.End_of_buffer -> error buf "unexpected end of block")
  | Varint _ | Varint64 _ -> IBuf.error buf "block length is too long"
  | _ -> assert false
...
let parse_toplevel_header buf =
  ...
  | None -> error buf "unexpected end of buffer when reading top-level header"
  | Some (field_code, field_value) ->
    if field_code = 1
    then field_value
...
let rec int_of_varint obj =
  ...
  | Varint64 x ->
    ...
    if int64_of_uint res <> x then error obj "int overflow in 'int_of_varint'";
    res
  | Top_block buf -> int_of_varint (parse_toplevel_header buf)
  | _ -> error obj "varint expected"
...
let rec int_of_signed_varint obj =
  ...
  | Varint64 x ->
    let res = Int64.to_int x in
    if Int64.of_int res <> x then error obj "int overflow in 'int_of_signed_varint'";
    res
  | Top_block buf -> int_of_signed_varint (parse_toplevel_header buf)
  | _ -> error obj "varint expected"

(* this encoding is only for signed integers *)
let int_of_zigzag_varint x = int_of_signed_varint (zigzag_varint_of_varint x)
let int_of_fixed32 x = Int32.to_int (expect_int32 x)
let int_of_fixed64 x = Int64.to_int (expect_int64 x)
...
(* this encoding is only for signed integers *)
let int64_of_zigzag_varint x = int64_of_signed_varint (zigzag_varint_of_varint x)
...
let rec int32_of_varint obj =
  ...
  | Varint64 x ->
    ...
    if int64_of_uint32 res <> x then error obj "int32 overflow in 'int32_of_varint'";
    res
...
(* this encoding is only for signed integers *)
let int32_of_zigzag_varint x = int32_of_signed_varint (zigzag_varint_of_varint x)
let int32_of_fixed32 = expect_int32
let int32_of_signed_fixed32 = int32_of_fixed32
let float_of_int32 x = Int32.float_of_bits x

(* XXX *)

let float_of_int64 x = Int64.float_of_bits x

(* XXX *)

let float_of_fixed64 buf = float_of_int64 (expect_int64 buf)
let float_of_fixed32 buf = float_of_int32 (expect_int32 buf)
...
let parse_string_field obj = validate_string (parse_binary_field obj)
...
let int_of_packed_varint buf = int_of_varint (try_parse_varint buf)
let int_of_packed_signed_varint buf = int_of_signed_varint (try_parse_varint buf)
let int_of_packed_zigzag_varint buf = int_of_zigzag_varint (try_parse_varint buf)
let int_of_packed_fixed32 buf = Int32.to_int (try_parse_fixed32 buf)
let int_of_packed_fixed64 buf = Int64.to_int (try_parse_fixed64 buf)
let int64_of_packed_varint buf = int64_of_varint (try_parse_varint buf)
let int64_of_packed_signed_varint buf = int64_of_signed_varint (try_parse_varint buf)
let int64_of_packed_zigzag_varint buf = int64_of_zigzag_varint (try_parse_varint buf)
let int64_of_packed_fixed64 buf = try_parse_fixed64 buf
...
let int64_of_packed_signed_fixed32 buf = Int64.of_int32 (try_parse_fixed32 buf)
let int32_of_packed_varint buf = int32_of_varint (try_parse_varint buf)
let int32_of_packed_signed_varint buf = int32_of_signed_varint (try_parse_varint buf)
let int32_of_packed_zigzag_varint buf = int32_of_zigzag_varint (try_parse_varint buf)
let int32_of_packed_fixed32 buf = try_parse_fixed32 buf
let int32_of_packed_signed_fixed32 = int32_of_packed_fixed32
let float_of_packed_fixed32 buf = float_of_int32 (try_parse_fixed32 buf)
let float_of_packed_fixed64 buf = float_of_int64 (try_parse_fixed64 buf)
let bool_of_packed_varint buf = bool_of_varint (try_parse_varint buf)
...
let parse_record_buf buf =
  let rec parse_unordered accu =
    match parse_field buf with
    | Some field -> parse_unordered (field :: accu)
    | None ->
...
let parse_record obj =
  match obj with
  | Block buf | Top_block buf -> parse_record_buf buf
  | obj -> error obj "block expected"
...
let parse_packed_field parse_packed_value parse_value obj =
  match obj with
  | Block buf -> parse_packed_fields parse_packed_value buf
  | _ -> [ parse_value obj ]
...
let parse_packed_array_field elem_size parse_packed_value buf =
  ...
  if size mod elem_size <> 0 then IBuf.error buf "invalid packed fixed-width field";
...
let parse_list_elem parse_value (code, x) =
  ...
  if code = 1 then parse_value x else error x "invalid list element code"
...
let parse_packed_list_1 parse_packed_value parse_value fields =
  rev_flatmap
    (parse_list_elem (parse_packed_field parse_packed_value parse_value))
    fields
...
let parse_packed_array_fixed elem_size parse_packed_value parse_value obj =
  let l = parse_record obj in
  match l with
  | [ (1, Block buf) ] -> parse_packed_array_field elem_size parse_packed_value buf
  | _ ->
...
module OBuf = struct
  ...
  let iol_known_size n l = Iol_size (n, l)
...
let gen_signed_varint_value x =
  ...
  if x < 0 then gen_varint64_value (Int64.of_int x) else gen_unsigned_varint_value x

let gen_unsigned_varint32_value x =
  let rec aux x =
    let b = Int32.to_int (Int32.logand x 0x7Fl) in
    (* base 128 *)
...
let gen_signed_varint_field code x =
  iol [ gen_primitive_key 0 code; gen_signed_varint_value x ]

let gen_varint_field code x =
  iol [ gen_primitive_key 0 code; gen_unsigned_varint_value x ]

let gen_signed_varint32_field code x =
  iol [ gen_primitive_key 0 code; gen_signed_varint32_value x ]

let gen_varint32_field code x =
  iol [ gen_primitive_key 0 code; gen_unsigned_varint32_value x ]

let gen_varint64_field code x = iol [ gen_primitive_key 0 code; gen_varint64_value x ]

(*
 * Generating fixed32 and fixed64 values and fields
 *)

let gen_fixed32_value x =
  (* little-endian *)
...
let gen_fixed32_field code x = iol [ gen_primitive_key 5 code; gen_fixed32_value x ]
let gen_fixed64_field code x = iol [ gen_primitive_key 1 code; gen_fixed64_value x ]
...
let zigzag_of_int x =
  ...
  (x lsl 1) lxor (x asr 62)

(* XXX: can use lesser value than 62 on 32 bit? *)
...
let int_to_varint code x = gen_varint_field code x
let int_to_signed_varint code x = gen_signed_varint_field code x
let int_to_zigzag_varint code x = gen_varint_field code (zigzag_of_int x)
let int64_to_varint code x = gen_varint64_field code x
let int64_to_signed_varint = int64_to_varint
let int64_to_zigzag_varint code x = int64_to_varint code (zigzag_of_int64 x)
let int64_to_fixed64 code x = gen_fixed64_field code x
let int64_to_fixed32 code x = gen_fixed32_field code (Int64.to_int32 x)
let int64_to_signed_fixed64 = int64_to_fixed64
let int64_to_signed_fixed32 = int64_to_fixed32
let int32_to_varint code x = gen_varint32_field code x
let int32_to_signed_varint code x = gen_signed_varint32_field code x
let int32_to_zigzag_varint code x = gen_varint32_field code (zigzag_of_int32 x)
let int32_to_fixed32 code x = gen_fixed32_field code x
let int32_to_signed_fixed32 = int32_to_fixed32
let int32_of_float x = Int32.bits_of_float x

(* XXX *)

let int64_of_float x = Int64.bits_of_float x

(* XXX *)

let float_to_fixed32 code x = gen_fixed32_field code (int32_of_float x)
let float_to_fixed64 code x = gen_fixed64_field code (int64_of_float x)

let int_of_bool = function
  | true -> 1
  | false -> 0

let bool_to_varint code x = gen_varint_field code (int_of_bool x)
let gen_bool_field = bool_to_varint

let gen_string_field code s =
  let contents = ios s in
  iol [ gen_primitive_key 2 code; gen_unsigned_varint_value (String.length s); contents ]
...
let gen_parsed_field (code, value) =
  match value with
  | Varint x -> gen_varint_field code x
  | Varint64 x -> gen_varint64_field code x
  | Int32 x -> gen_fixed32_field code x
  | Int64 x -> gen_fixed64_field code x
  | Block x ->
    iol [ gen_primitive_key 2 code; gen_unsigned_varint_value (IBuf.size x); IBuf x ]
  | Top_block x ->
    (* impossible clause *)
    assert false

let gen_parsed_field_list l = List.map gen_parsed_field l
...
let int_to_packed_varint x = gen_unsigned_varint_value x
let int_to_packed_signed_varint x = gen_signed_varint_value x
let int_to_packed_zigzag_varint x = gen_unsigned_varint_value (zigzag_of_int x)
let int64_to_packed_varint x = gen_varint64_value x
let int64_to_packed_signed_varint x = gen_varint64_value x
let int64_to_packed_zigzag_varint x = gen_varint64_value (zigzag_of_int64 x)
let int64_to_packed_fixed64 x = gen_fixed64_value x
let int64_to_packed_fixed32 x = gen_fixed32_value (Int64.to_int32 x)
let int64_to_packed_signed_fixed64 = int64_to_packed_fixed64
let int64_to_packed_signed_fixed32 = int64_to_packed_fixed32
let int32_to_packed_varint x = gen_unsigned_varint32_value x
let int32_to_packed_signed_varint x = gen_signed_varint32_value x
let int32_to_packed_zigzag_varint x = gen_unsigned_varint32_value (zigzag_of_int32 x)
let int32_to_packed_fixed32 x = gen_fixed32_value x
let int32_to_packed_signed_fixed32 = int32_to_packed_fixed32
let float_to_packed_fixed32 x = gen_fixed32_value (int32_of_float x)
let float_to_packed_fixed64 x = gen_fixed64_value (int64_of_float x)
let bool_to_packed_varint x = gen_unsigned_varint_value (int_of_bool x)
...
let gen_repeated_field code f l = iol (List.map (f code) l)
...
let gen_repeated_array_field code f l = iol (map_a2l (f code) l)
...
let gen_packed_repeated_field_common code contents =
  ...
  else iol [ gen_key 2 code; gen_unsigned_varint_value size; contents ]
 ...
 * parse_block() below *)
let gen_block iodata = iol [ gen_unsigned_varint_value (OBuf.size iodata); iodata ]

(* XXX, TODO: return Some or None on End_of_buffer *)
let parse_block buf = Top_block (parse_block buf)
