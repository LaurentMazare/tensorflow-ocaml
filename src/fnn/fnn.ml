...
let input ~shape =
  let id = Id.create () in
  { shape; op = Input; id }, id

let const f ~shape = { shape; op = Const f; id = Id.create () }
let unary unary t = { shape = shape t; op = Unary (unary, t); id = Id.create () }
...
let binary binary t1 t2 =
  ...
  { shape = shape t1; op = Binary (binary, t1, t2); id = Id.create () }

let reshape t ~shape = { shape; op = Reshape (shape, t); id = Id.create () }
let flatten t = reshape t ~shape:(D1 (Shape.total_dim t.shape))
...
let concat = function
  ...
  | hd :: _ as l ->
    ...
    { shape = D2 (List.length l, hd_shape); op = Concat l; id = Id.create () }

let var t = { shape = t.shape; op = Var t; id = Id.create () }

let dense' ?(w_init = `const 0.) ?(b_init = `const 0.) ?name dim =
  let id = Id.create () in
  Staged.stage (fun t -> { shape = D1 dim; op = Dense (w_init, b_init, t, name); id })

let dense ?w_init ?b_init ?name dim = Staged.unstage (dense' ?w_init ?b_init ?name dim)
...
let pool ~avg_or_max t ~filter ~strides ~padding =
  ...
  let output_height, output_width =
    conv_sizes
      ...
      ~padding
  in
  let pool = { filter; strides; padding; avg_or_max } in
...
let conv2d ?w_init ?b_init ?name ~filter ~out_channels ~strides ~padding () =
  Staged.unstage
    (conv2d' ?w_init ?b_init ?name ~filter ~out_channels ~strides ~padding ())
    ...
    let node =
          ...
          Hashtbl.find_or_add dense_vars t.id ~default:(fun () ->
              ...
              Option.iter name_opt ~f:(fun name ->
                  Hashtbl.set
                    var_names
                    ~key:(Node.id w)
                    ~data:(name ^ "/" ^ name ^ "_weights");
                  Hashtbl.set
                    var_names
                    ~key:(Node.id b)
                    ~data:(name ^ "/" ^ name ^ "_biases"));
              w, b)
        in
      ...
      | Conv2d (conv2d, u) ->
        ...
        then
          shape_mismatch
            (D1 in_channels)
            (D1 conv2d.in_channels)
            ~op_name:"conv2d in-channels";
      ...
      | Concat list -> List.map list ~f:(fun t -> walk (P t)) |> Ops.(concat one32)
      ...
      | Var u ->
        Hashtbl.find_or_add explicit_vars t.id ~default:(fun () ->
            ...
            Var.create dim_list ~type_ ~init:(walk (P u)))
    in
...
module Optimizer = struct
  ...
  let momentum ~learning_rate ~momentum = Momentum (learning_rate, momentum)
...
module Loss = struct
  ...
  let get t ~sample_ys ~model_ys =
    ...
    in
    match t with
    | Cross_entropy sum_mean -> Ops.(neg (reduce sum_mean (sample_ys * log model_ys)))
    | L2 sum_mean -> Ops.(reduce sum_mean (square (sample_ys - model_ys)))
end
...
module Model = struct
  ...
  let create (type a b) (eq : (b * a) Tensor.eq) fnn =
    let create eq ~type_ =
      let node, inputs, var_names, explicit_vars, all_nodes =
        build_node (P fnn) ~type_
      in
  ...
  let fit
    ...
    let fit placeholder node f_or_d scalar_f_or_d =
      let loss =
        Loss.get loss ~sample_ys:(Ops.Placeholder.to_node placeholder) ~model_ys:node
      in
      let optimizer =
        let varsf, varsd =
      ...
      for epoch = 1 to epochs do
        ...
        for step = 0 to batches_per_epoch - 1 do
          ...
          let err = Session.run ~inputs ~targets:optimizer (scalar_f_or_d loss) in
        ...
        Stdio.printf
          ...
          epoch
          epochs
          (!err_total /. Float.of_int batches_per_epoch)
      done
    in
  ...
  let input_list
    ...
    List.map inputs ~f:(fun (id, tensor) ->
        ...
        | Some placeholder ->
          (match Node.output_type t.node, t.eq with
          | Node.Type.Float, Tensor.Float -> Session.Input.float placeholder tensor
          | Node.Type.Double, Tensor.Double -> Session.Input.double placeholder tensor
          | _ -> .))
  ...
  let save ?(inputs = []) t ~filename =
    let save_node =
      Hashtbl.find_or_add t.save_nodes filename ~default:(fun () ->
          ...
          if List.is_empty all_vars_with_names
          then
